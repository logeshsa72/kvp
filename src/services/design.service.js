...
async function getOne(id) {
    ...
    if (!data) return NoRecordFound("design");
...
async function update(id, body) {
    ...
    const dataFound = await prisma.design.findUnique({
        where: {
            id: parseInt(id)
        }
    })
    if (!dataFound) return NoRecordFound("design");
    const data = await prisma.design.update({
        where: {
            id: parseInt(id),
        },
        data:
...
async function remove(id) {
    const data = await prisma.design.delete({
        where: {
            id: parseInt(id)
        },
    })
