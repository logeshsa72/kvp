...
async function create(body) {
    const { name, code, contactEmail, contactName, contactMobile, companyId } = await body
    const data = await prisma.branch.create({
        data: {
            branchName: name, branchCode: code, contactEmail, contactName, contactMobile: parseInt(contactMobile),
            company: {
...
async function update(id, body) {
    const { name, code, contactEmail, contactName, contactMobile, active, prefixCategory, idPrefix, idSequence, tempPrefix, tempSequence } = await body
    ...
    const data = await prisma.branch.update({
        ...
        data: {
            ...
            idPrefix, idSequence, tempPrefix, tempSequence, prefixCategory
        },
    })
    return { statusCode: 0, data };
};
